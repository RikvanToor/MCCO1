module Types.Synonym where

import Types.Primitive
import Types.Substitution hiding (lookupInt)
...
getTypeSynonymOrdering synonyms =
   let
       ...
       err          = internalError "Types.Synonyms" "getTypeSynonymOrdering" "error in lookup table"
   ...
   case leftSpine tp of
      (TCon s, tps) -> case M.lookup s synonyms of
                          Just (i, f) | i == length tps -> Just (f tps)
                                      | otherwise       -> internalError "Types.Synonyms"
...
expandOneStepOrdered (ordering, synonyms) (t1,t2) =
   let f tp = case fst (leftSpine tp) of
       ...
       err = internalError "Types.Synonyms" "expandOneStep" "invalid set of OrderedTypeSynonyms"
